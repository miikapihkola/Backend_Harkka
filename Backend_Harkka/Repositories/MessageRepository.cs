...
namespace Backend_Harkka.Repositories
{
    public class MessageRepository : IMessageRepository
    {
        ...
        {            
            long itemCount = (await _context.Messages.Where(x => x.Recipient == null).Where(x => x.IsDeleted == false).ToListAsync()).Count;
        ...
        {
            long itemCount = (await _context.Messages.Where(x => x.Recipient == user).Where(x => x.IsDeleted == false).ToListAsync()).Count;
            int pageOut = ConfirmPage(itemCount, page);
        ...
        {
            long itemCount = (await _context.Messages.Where(x => x.Sender == user).Where(x => x.IsDeleted == false).ToListAsync()).Count;
            int pageOut = ConfirmPage(itemCount, page);
